...
- Required secrets: `pix.secrets.BTG_CLIENT_ID`, `BTG_CLIENT_SECRET`, `INTERNAL_WEBHOOK_SECRET`, `MIDAZ_CLIENT_ID`, `MIDAZ_CLIENT_SECRET`, and `LICENSE_KEY`. Infra DB credentials are single-sourced (see below) and are NOT required on the default bundled install.
- Dependency notes: Uses local PostgreSQL (Bitnami), MongoDB (Bitnami), and Valkey (`valkey.io/valkey`) dependency charts unless external services are configured.
- Single-source infra secrets: On the bundled path the PostgreSQL and MongoDB passwords are auto-generated by the subcharts into their own Secrets and read by the app via `secretKeyRef` — leave `*.secrets.DB_PASSWORD` / `DB_REPLICA_PASSWORD` / `MONGO_PASSWORD` and `postgresql.auth.password` / `mongodb.auth.rootPassword` EMPTY. When PostgreSQL or MongoDB is external/disabled the render fails loud unless you set the matching `*.secrets.DB_PASSWORD` / `MONGO_PASSWORD` (or a `postgresql.auth.existingSecret` / `mongodb.auth.existingSecret`). `REDIS_PASSWORD` is always operator-provided — `valkey.io/valkey` exposes no Secret-based password to source from.
- Production overrides: Provide BTG, Midaz, and license credentials through chart secrets or existing Secrets; override PIX/inbound/outbound/reconciliation image tags, ingress, resources, and persistence.
...
```console
$ helm install plugin-br-pix-indirect-btg oci://registry-1.docker.io/lerianstudio/plugin-br-pix-indirect-btg-helm --version <version> -n midaz-plugins --create-namespace
```
...
> Note on database passwords (single-source): with the bundled PostgreSQL and MongoDB subcharts
> you do NOT set `DB_PASSWORD`, `DB_REPLICA_PASSWORD`, `MONGO_PASSWORD`, `postgresql.auth.password`,
> or `mongodb.auth.rootPassword`. The subcharts auto-generate these into their own Secrets and the
> application reads them via `secretKeyRef`. Set them only when pointing the chart at external
> PostgreSQL/MongoDB (`postgresql.enabled=false` / `mongodb.enabled=false`), where the render fails
> loud if the credential is missing.

Then install with your custom values:

```console
$ helm install plugin-br-pix-indirect-btg oci://registry-1.docker.io/lerianstudio/plugin-br-pix-indirect-btg-helm --version <version> -f values-custom.yaml -n midaz-plugins --create-namespace
```
...
| `pix.image.tag` | Image tag used for deployment. | `"1.6.0"` |
...
| `inbound.image.tag` | Image tag used for deployment. | `"1.6.0"` |
...
| `outbound.image.tag` | Image tag used for deployment. | `"1.6.0"` |
...
- **Note on credentials:** `valkey.io/valkey` is NOT the Bitnami Valkey chart and exposes no
  Secret-based password (auth ships disabled). `REDIS_PASSWORD` is therefore always operator-provided
  via `*.secrets.REDIS_PASSWORD` — there is no subchart Secret to single-source it from. The bundled
  Service host is derived automatically (collapse-aware); set `REDIS_HOST` only for external Valkey/Redis.
...
- **Note on credentials (single-source):** When bundled, the subchart auto-generates the password into
  its own Secret (`<release>-postgresql`, key `password`) and the app reads it via `secretKeyRef` — leave
  `postgresql.auth.password` and the `*.secrets.DB_PASSWORD` / `DB_REPLICA_PASSWORD` keys EMPTY. The DB
  host is derived collapse-aware from the subchart Service name. When external/disabled you MUST set
  `*.secrets.DB_PASSWORD` (and the `DB_HOST`/`DB_REPLICA_HOST`), or set `postgresql.auth.existingSecret`;
  otherwise the render fails loud. PostgreSQL is standalone here, so the replica host/credentials default
  to the primary.
...
- **Note on credentials (single-source):** The app connects to MongoDB as the root user
  (`mongodb.auth.rootUser: pix_btg`). When bundled, the subchart auto-generates the root password into
  its own Secret (`<release>-mongodb`, key `mongodb-root-password`) and the app reads it via `secretKeyRef`
  — leave `mongodb.auth.rootPassword` and `*.secrets.MONGO_PASSWORD` EMPTY. The Mongo host is derived
  collapse-aware. When external/disabled you MUST set `*.secrets.MONGO_PASSWORD` (and `MONGO_HOST`), or
  set `mongodb.auth.existingSecret`; otherwise the render fails loud. (The optional
  `global.externalMongoDefinitions` bootstrap Job is a separate tool for provisioning an application user
  on an external MongoDB and is disabled by default.)

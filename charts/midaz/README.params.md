...
|-----|------|---------|-------------|
| `global.externalRabbitmqDefinitions` | object | `{}` | Bootstrap job for external RabbitMQ: creates users, vhosts, and permissions |
| `global.externalRabbitmqDefinitions.enabled` | bool | `false` | Enable or disable the RabbitMQ bootstrap job |
| `global.externalRabbitmqDefinitions.connection` | object | `{}` | RabbitMQ connection settings |
...
| `global.externalRabbitmqDefinitions.rabbitmqAdminLogin` | object | `{}` | Admin credentials for RabbitMQ management API |
...
| `global.externalRabbitmqDefinitions.appCredentials` | object | `{}` | Credentials for application users created by the job |
...
| `global.externalPostgresDefinitions` | object | `{}` | Bootstrap job for external PostgreSQL: creates databases, roles, and grants privileges |
| `global.externalPostgresDefinitions.enabled` | bool | `false` | Enable or disable the PostgreSQL bootstrap job |
| `global.externalPostgresDefinitions.connection` | object | `{}` | PostgreSQL connection settings |
...
| `global.externalPostgresDefinitions.postgresAdminLogin` | object | `{}` | Admin credentials for PostgreSQL |
...
| `global.externalPostgresDefinitions.midazCredentials` | object | `{}` | Credentials for midaz role created by the job |
...
| `global.externalMongoDefinitions.midazCredentials` | object | `{}` | Credentials for midaz user created by the job |
...
| `global.externalMongoDefinitions.midazCredentials.roles` | object | `{}` | List of roles to grant to the user. Each item is a {role, db} pair. |
...
| `ledger.migrations` | object | `{}` | Dedicated migration runner (`midaz-ledger-migrations`, built FROM migrate/migrate) applying the onboarding and transaction migration sets. |
| `ledger.migrations.enabled` | object | `{}` | Run the migration Job. Unset means auto: on for 4.x ledger tags, which removed in-process migration, off for 3.x, which still migrates at startup. Set false to keep it off when the schema is applied out-of-band. |
...
| `ledger.migrations.waitForPostgres` | object | `{}` | Wait for the PostgreSQL endpoint before running migrate |
...
| `ledger.migrations.resources.limits` | object | `{}` | CPU and memory limits for the migration Job |
| `ledger.migrations.resources.requests` | object | `{}` | Minimum CPU and memory requests for the migration Job |
...
| `ledger.resources.limits` | object | `{}` | CPU and memory limits for pods |
| `ledger.resources.requests` | object | `{}` | Minimum CPU and memory requests |
...
| `crm.enabled` | bool | `false` | Enable or disable the CRM service |
| `crm.replicaCount` | int | `1` | Number of replicas for the CRM service |
| `crm.revisionHistoryLimit` | int | `10` | Number of old ReplicaSets to retain for deployment rollback |
| `crm.image.repository` | string | `lerianstudio/midaz-crm` | Repository for the CRM service container image |
| `crm.image.pullPolicy` | string | `Always` | Image pull policy |
| `crm.image.tag` | string | `"3.8.0"` | Image tag used for deployment |
| `crm.imagePullSecrets` | list | `[]` | Secrets for pulling images from a private registry |
| `crm.nameOverride` | string | `""` | Overrides the default generated name by Helm |
| `crm.fullnameOverride` | string | `""` | Overrides the full name generated by Helm |
| `crm.podAnnotations` | object | `{}` | Pod annotations for additional metadata |
| `crm.securityContext.runAsGroup` | int | `1000` | Defines the group ID for the user running the process inside the container |
| `crm.securityContext.runAsUser` | int | `1000` | Defines the user ID for the process running inside the container |
| `crm.securityContext.runAsNonRoot` | bool | `true` | Ensures the process does not run as root |
...
| `crm.deploymentUpdate.maxSurge` | int | `1` | Maximum number of pods that can be created over the desired number of pods |
| `crm.deploymentUpdate.maxUnavailable` | int | `1` | Maximum number of pods that can be unavailable during the update |
| `crm.service.type` | string | `ClusterIP` | Kubernetes service type |
| `crm.service.port` | int | `4003` | Service port |
| `crm.ingress.enabled` | bool | `false` | Enable or disable ingress |
| `crm.ingress.className` | string | `""` | Ingress class name |
| `crm.ingress.annotations` | object | `{}` | Additional ingress annotations |
| `crm.ingress.tls` | list | `[]` | TLS configuration for ingress |
| `crm.resources.limits` | string | `{}` | CPU and memory limits for pods |
| `crm.resources.requests` | string | `{}` | Minimum CPU and memory requests |
| `crm.autoscaling.enabled` | bool | `true` | Enable or disable horizontal pod autoscaling |
...
| `crm.datastores` | object | `templates/crm/configmap.yaml` | Dedicated datastore mask (override global.datastores). Sub-key: mongo (host/port/user). |
| `crm.configmap` | object | `{}` | Escape hatch: any native env var overrides the template default. |
| `crm.extraEnvVars` | object | `{}` | Extra environment variables |
| `crm.secrets` | string | `templates/crm/secrets.yaml` | Secrets for storing sensitive data |
| `crm.secrets.LCRYPTO_HASH_SECRET_KEY` | string | `""` | App crypto material (operator-provided, external boundary). |
| `crm.secrets.MONGO_PASSWORD` | string | `""` | MONGO_PASSWORD is single-sourced from the Bitnami mongodb subchart Secret (key `mongodb-root-password`); only set it for an EXTERNAL MongoDB (subchart disabled / .external=true) without an existingSecret override. |
| `crm.secrets.SD_TOKEN` | string | `""` | Service discovery (lib-service-discovery) Consul ACL token. Operator-provided; only set when the Consul server enforces ACLs. Rendered into the CRM Secret. |
| `crm.secrets.STREAMING_SASL_PASSWORD` | string | `""` | Streaming (lib-streaming) SASL/TLS material. Operator-provided; only set when the streaming backend uses SASL auth and/or a private TLS CA. When set, they are rendered into the CRM Secret (never the ConfigMap). |
| `crm.useExistingSecret` | bool | `false` | Existing secrets name |
...
| `tracer.migrations` | object | `{}` | Dedicated migration runner (`midaz-tracer-migrations`, built FROM migrate/migrate). The v4 tracer no longer migrates at startup: it boots against an already-migrated schema, so this Job has to apply it. |
...
| `tracer.migrations.waitForPostgres` | object | `{}` | Wait for the PostgreSQL endpoint before running migrate |
...
| `tracer.migrations.resources.limits` | object | `{}` | CPU and memory limits for the migration Job |
| `tracer.migrations.resources.requests` | object | `{}` | Minimum CPU and memory requests for the migration Job |
...
| `tracer.resources.limits` | object | `{}` | CPU and memory limits for pods |
| `tracer.resources.requests` | object | `{}` | Minimum CPU and memory requests |
...
| `tracer.secrets` | object | `templates/tracer/secrets.yaml` | Secrets for storing sensitive data |
...
| `grafana.ingress` | object | `{}` | Configure the ingress for Access Grafana Dashboard |
| `otel-collector-lerian` | object | `{}` | OTEL exporter configuration for midaz services. When enabled, HOST_IP, POD_IP, OTEL_EXPORTER_OTLP_ENDPOINT and OTEL_RESOURCE_ATTRIBUTES are injected into the `ledger` and `crm` deployments only. Port is fixed at 4317. If you need a different collector endpoint, leave enabled=false and configure OTEL env vars directly in the application configmap (ledger.configmap / crm.configmap). |

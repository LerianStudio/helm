...
```console
$ helm install midaz oci://registry-1.docker.io/lerianstudio/midaz-helm --version 3.0.0 -n midaz --create-namespace
```
...
- **Note:** This niginx proxy manager is used to configure the ingress for the plugins UIs. 
...
- **Note:** This otell collector is used to collect metrics from the Midaz components.

 - By default, it is disabled.
 - You can enable it by setting `otell.enabled` to `true` in the values file.

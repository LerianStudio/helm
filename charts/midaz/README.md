...
The Midaz Helm Chart supports different Ingress Controllers for exposing services. You can enable Ingress for the following services: Ledger and Console. Below are configurations for commonly used controllers.
...
| `ledger.image.tag` | Image tag used for deployment. | `"3.7.2"` |
...
**Note:** The ledger service uses module-specific database credentials (onboarding and transaction) since it combines both modules. The `DB_*`, `MONGO_*`, and `REDIS_PASSWORD` keys above are only read when the matching backend is **external**; for the bundled Bitnami subcharts those passwords are single-sourced from the subchart Secret via `secretKeyRef`, so the keys in this secret are ignored. Only `RABBITMQ_DEFAULT_PASS`, `RABBITMQ_CONSUMER_PASS`, and (for external backends) the infra passwords are required.
...
| `crm.image.tag` | Image tag used for deployment. | `"3.7.2"` |
...
- **Database.** Tracer uses the bundled PostgreSQL with the same `midaz` role that owns `onboarding` and `transaction`. Its database (`tracer.configmap.DB_NAME`, default `tracer`) is created by `files/midaz/init.sql` on a fresh internal cluster, and by the external bootstrap Job (`global.externalPostgresDefinitions.enabled=true`) otherwise; both paths use the configured name. On a pre-existing internal cluster, initdb scripts no longer run: create the configured database once by hand before enabling tracer (`CREATE DATABASE tracer;` with the default `tracer.configmap.DB_NAME`).

...
| `global.auth` | object | `{}` | Env-wide inbound auth (plugin-access-manager), consumed by lerian-common.globalValue (block "auth"). A component brCcs.configmap.PLUGIN_AUTH_* still overrides. Leave empty ({}) for the chart defaults (PLUGIN_AUTH_ENABLED=false, HOST unset). |
...
| `brCcs.configmap` | object | `templates/configmap.yaml` | PRIMARY override surface (escape hatch). TIERED model: dependency connections (postgres/redis/broker, multiTenant, otel, serviceDiscovery, streaming, auth) are the typed, documented knobs — the datastore mask (brCcs.datastores + global.datastores) and the global.* dependency blocks. EVERYTHING ELSE (rate-limit, outbox, swagger, cors, pagination, fetcher/sta/ reporter integration, object storage, probes, timeouts, retention, tuning) has its default in templates/configmap.yaml and is reached ONLY here: set a raw NATIVE_KEY to override it, or to pass an optional/opt-in key (e.g. SERVER_TLS_CERT_FILE, RABBITMQ_QUEUE, FETCHER_URL, *_OAUTH2_*, CCS_DETAIL_*). Precedence: brCcs.configmap.<KEY> > chart default. Leaving this {} renders the documented defaults (byte-identical to the app .env). |

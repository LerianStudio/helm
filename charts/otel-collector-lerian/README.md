...
- **transform/normalize_http_semconv**: Bidirectional mirror between legacy HTTP attrs (`http.method`, `http.status_code`) and OTel stable (`http.request.method`, `http.response.status_code`). Keeps both forms populated on every span during the lib-observability migration window (only `trace_statements`; logs are not normalized because the access log fields use underscore form `http_method`/`http_status_code` and are parsed by Loki directly). Skips when the new method attr is `_OTHER` (cardinality protection sentinel).

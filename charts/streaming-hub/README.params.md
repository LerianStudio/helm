...
## Global (env-wide, shared across components / the umbrella)

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `global.datastores` | object | `{}` | Shared datastore masks (lerian-common.datastore.value). Operator sets the typed fields instead of raw env; empty by default so the template defaults win (standalone stays render-equivalent). Dedicated per-component overrides live under streamingHub.datastores. Example: postgres: { host: pg.internal, port: "5432", user: streaming_hub, name: streaming_hub, ssl: require } |
| `global.auth` | object | `{}` | Shared plugin-access-manager JWT auth contract (lerian-common.auth.env). |
| `global.auth.enabled` | bool | `true` | Enable control-plane JWT auth env-wide (PLUGIN_AUTH_ENABLED). Override per component via streamingHub.configmap.PLUGIN_AUTH_ENABLED. |
| `global.observability` | object | `{}` | Shared observability contract (lerian-common.globalValue block `observability`). Empty by default → the template ships the defaults. Fields: otlpEndpoint (OTEL_EXPORTER_OTLP_ENDPOINT), insecureExporter (OTEL_INSECURE_EXPORTER), deploymentEnvironment (ENV_NAME). The per-pod OTLP endpoint override is governed separately by streamingHub.telemetry.enabled. |
| `global.multiTenant` | object | `{}` | Shared multi-tenant contract (lib-commons/multitenancy). `enabled` drives the UNPREFIXED MULTI_TENANT_ENABLED knob; override per component via streamingHub.configmap.MULTI_TENANT_ENABLED. streaming-hub's MT model reads tenant credentials from AWS Secrets Manager, so it needs no tenant-manager URL and no tenant-registry Redis (MULTI_TENANT_REDIS_* stays unset). The hub's OWN Redis/Valkey — STREAMING_HUB_REDIS_* — is a SEPARATE connection used only for control-plane rate limiting; it is not part of this tenancy contract. |
| `global.externalPostgresDefinitions` | object | `{}` | Bootstrap Job for an external/shared PostgreSQL: creates the hub's ONE database + role and grants privileges. The hub owns a SINGLE database with a tenant_id column (NOT per-tenant DB). Default OFF — dev-st may instead point STREAMING_HUB_POSTGRES_DSN at a pre-provisioned managed host. |
...
| `global.externalPostgresDefinitions.connection` | object | `{}` | PostgreSQL connection settings for the bootstrap Job. |
...
| `global.externalPostgresDefinitions.postgresAdminLogin` | object | `{}` | Admin credentials used by the Job to create the DB/role. |
...
| `global.externalPostgresDefinitions.hubCredentials` | object | `{}` | Credentials for the hub role created by the Job. |
...
| `streamingHub.secrets.STREAMING_HUB_KAFKA_SCRAM_PASSWORD` | string | `""` | SASL/SCRAM password (emit-when-set; required when KAFKA_SCRAM_MECHANISM is set). The SCRAM username is non-sensitive and lives in the ConfigMap. |
| `streamingHub.secrets.POSTGRES_PASSWORD` | string | `""` | Migration-toolchain Postgres password (emit-when-set; no in-cluster consumer — the migration Job uses the DSN — shipped for .env parity). |
| `streamingHub.secrets.STREAMING_HUB_REDIS_PASSWORD` | string | `""` | Rate-limiting Redis/Valkey password (emit-when-set; MAY stay empty when the instance takes no AUTH). The host, port, TLS flag and CA cert are NOT credentials and live in the ConfigMap as STREAMING_HUB_REDIS_ADDRESS / _TLS / _CA_CERT. |
...
| `aws.rolesAnywhere.enabled` | bool | `false` | Enable the IAM Roles Anywhere credential sidecar. |
| `aws.rolesAnywhere.trustAnchorArn` | string | `""` | Trust anchor ARN (required when enabled). |
| `aws.rolesAnywhere.profileArn` | string | `""` | Profile ARN (required when enabled). |
| `aws.rolesAnywhere.roleArn` | string | `""` | Role ARN the sidecar assumes (required when enabled). |
| `aws.rolesAnywhere.region` | string | `"us-east-2"` | AWS region for the credential exchange. |
| `aws.rolesAnywhere.sessionDuration` | int | `3600` | Credential session duration in seconds. |
| `aws.rolesAnywhere.certificateSecretName` | string | `""` | Name of the kubernetes.io/tls Secret holding tls.crt / tls.key. Left empty on purpose so the template fallback in _deployment.tpl actually fires: it resolves to "<streaming-hub.fullname>-iam-tls". Note that fullname is fullnameOverride if set, else nameOverride if set, else the CHART name (never the Helm release name), so the fallback yields "streaming-hub-iam-tls" unless one of those overrides is set (e.g. nameOverride: foo -> "foo-iam-tls"). A non-empty default here made that branch unreachable and pinned every install to one literal name, ignoring the overrides. |
| `aws.rolesAnywhere.sidecar.port` | int | `9911` | Port the sidecar serves the IMDS-compatible endpoint on. |

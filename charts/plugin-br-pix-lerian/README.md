...
| `ORGANIZATION_ID` | `configmap` | Application boot | Single-tenant business identity. Read by six workloads: `spi`, `dictHub`, `dictHubVsync`, `dictProxy`, `cobHub` and `pixauto`. In single-tenant mode the workload reads its domain's `organization_id` from the Systemplane store first and falls back to this value when the store holds nothing — so on `pixauto`, with neither the store key nor this variable set, every request answers `403 TENANT_CONFIG_NOT_FOUND`. **Must be empty** when `MULTI_TENANT_ENABLED=true` on `cobHub` and `pixauto` — a value there refuses the boot, because a deployment-wide organization could only act as a cross-tenant fallback. Not interchangeable with `ORGANIZATION_IDS`. |

...
var allowlistedCredentialDefaults = map[string]bool{
	...
	// midaz bundles an optional dev-mode Vault (vault.enabled, default off / opt-in) so an
	// operator can spin up a self-contained standalone stack like the compose infra. When
	// enabled, its dev root token must be "root" to match the app's built-in DefaultVaultDevToken
	// (KMS_VAULT_AUTH_METHOD=token, DEPLOYMENT_MODE=local); it is a throwaway in-memory dev token,
	// not a real credential. Production leaves vault.enabled=false and points KMS_VAULT_ADDR at an
	// external Vault.

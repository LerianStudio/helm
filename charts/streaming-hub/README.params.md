...
| `global.multiTenant` | object | `{}` | Shared multi-tenant contract (lib-commons/multitenancy). `enabled` drives the UNPREFIXED MULTI_TENANT_ENABLED knob; override per component via streamingHub.configmap.MULTI_TENANT_ENABLED. streaming-hub's MT model reads tenant credentials from AWS Secrets Manager (no tenant-manager URL / Redis). |

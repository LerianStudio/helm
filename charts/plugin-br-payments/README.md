...
| `app.secrets.BTG_WEBHOOK_HMAC_SECRET` | HMAC-SHA256 signing key for incoming BTG settlement webhooks: the plugin recomputes the signature over the raw request body under this key, with no second factor. At least 32 characters. NEVER reuse `BTG_WEBHOOK_SECRET` as this value — the bearer token travels in a request header on every delivery and survives in edge/WAF/APM logs, so reusing it here would let anyone who reads those logs forge signed settlement events indefinitely. |
